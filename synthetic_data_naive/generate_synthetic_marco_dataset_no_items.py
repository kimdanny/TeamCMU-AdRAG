...
PATH_TO_ANSWERS = f"/you_path/Qwen2.5-0.5B-bidirectional-attn-mntp-marco-passage-hard-negatives-matrioshka-reduction-2/Qwen2.5-7B-Instruct-10-passage-RAG_{task_id}.jsonl"
PATH_TO_QUERIES = "/your_path/marco_v2.1_qa_train/queries.tsv"

# Output folder for individual tasks
OUTPUT_FOLDER = f"/your_path/Qwen2.5-0.5B-bidirectional-attn-mntp-marco-passage-hard-negatives-matrioshka-reduction-2/Qwen2.5-7B-Instruct-10-passage-RAG/prompt1-12"
...
def generate_prompt(query, response, mode="random"):
    if mode == "fixed":
        prompt_file = "/your_path/prompts/5.prompt"
        with open(prompt_file, "r") as file:
            prompt_template = file.read()
    elif mode == "random":
        prompt_files = [f"/your_path/prompts/{i}.prompt" for i in range(1, 13)]
...
output_file = os.path.join(
    OUTPUT_FOLDER, f"{model_name.split('/')[-1]}_{task_id}_with_ad.jsonl"
)
with open(output_file, "w", encoding="utf-8") as f:
    for output, entry in zip(outputs, data_entries):
        ...
        f.write(
            json.dumps(
                {"query_id": query_id, "without_ad": original, "with_ad": with_answer}
            )
            + "\n"
        )
