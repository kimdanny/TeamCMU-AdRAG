...
def read_file(file_path):
    ...
    with open(file_path, "r") as f:
        for line in f:
            ...
            responses = {"id": data["id"], "generated_text": data["generated_text"]}
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Predict labels for generated responses."
    )
    parser.add_argument(
        "--model-dir",
        type=str,
        default="jmvcoelho/ad-classifier-v0.2",
    ...
    for filename in tqdm(os.listdir(input_dir)):
        if filename.endswith(".jsonl") or filename.endswith(".json"):
            ...
            for item in tqdm(rewritten_responses, desc="Predicting labels"):
                ...
                predictions.append(
                    {"id": item["id"], "predicted_label": predicted_label}
                )
            ...
            with open(
                f"{output_dir}/predicted_label_{output_path}.jsonl", "w"
            ) as outfile:
            ...
            print(
                f"Predictions saved to {output_dir}/predicted_label_{output_path}.jsonl"
            )
